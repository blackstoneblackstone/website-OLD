<?php
/***********************************************************
	Filename: app/admin/control/blcok.php
	Note	: 数据调用配置页，支持自定义代码
***********************************************************/
class block_c extends Control
{
	var $module_sign = "block";
	...
	function index_f()
	{
		...
		$this->block_m->langid($_SESSION["sys_lang_id"]);
		$pageid = $this->trans_lib->int(SYS_PAGEID);
		$keywords = $this->trans_lib->safe("keywords");
		$page_url = site_url("block");
		$condition = "intype='block'";
		if($keywords)
		{
			$this->tpl->assign("keywords",$keywords);
			$page_url.="keywords=".rawurlencode($keywords)."&";
			$condition  .= "  and title LIKE '%".$keywords."%' OR note LIKE '%".$keywords."%' ";
		}
		$rslist = $this->block_m->get_list($pageid,$condition);
		$this->tpl->assign("rslist",$rslist);
		$count = $this->block_m->get_count($condition);
		$this->tpl->assign("total",$count);
		$pagelist = $this->page_lib->page($page_url,$count);
		$this->tpl->assign("pagelist",$pagelist);
		$this->tpl->display("block/list.html");
	}
	...
	function set_f()
	{
		...
		$this->tpl->display("block/set.html");
	}
	...
	function chk_identifier_f()
	{
		...
		//判断这个标识符是否有被使用
		$this->block_m->langid($_SESSION["sys_lang_id"]);
		$if_used = $this->block_m->chk_identifier($val);
		if($if_used)
		{
			exit("error: 标识串已经被使用");
		}
	...
	function setok_f()
	{
		...
		elseif($intype == "html" || $intype == "block")
		{
			$maxcount = 1;
		...
		//存储分类信息
		$this->block_m->save($array,$id);
		error("信息添加/存储成功",site_url("block"));
	}
	...
	function ajax_status_f()
	{
		...
		$this->block_m->set_status($id,$status);
		exit("ok");
	}
