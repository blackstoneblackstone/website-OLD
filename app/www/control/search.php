...
class search_c extends Control {
    ...
    function index_f() {
        ...
        //取得内容
		if($_POST['start'])
		   $list_condition .= ' and post_date>'.strtotime($_POST['start']);
		if($_POST['end'])
		   $list_condition .= ' and post_date<'.strtotime($_POST['end']);
	...
	/** add by EricBao */
    function export_f() {
    $file = fopen('downloaded.csv', 'w');
    $title=array('日期','购进单位','商品名称','生产厂家','商品规格','数量','单价','金额','批号');
    fputcsv($file, $title);
		...
		if($_POST['end'])
		   $list_condition .= ' and post_date<'.strtotime($_POST['end']);
		   $this->search_m->set_max(3000);
		   $this->search_m->set_psize(3000);
		$this->search_m->get_all_id($list_condition,$ext_keywords);
		$rslist = $this->search_m->get_list();
        if ($rslist) {
            foreach ($rslist as $key => $value) {
                $value2['post_date']=date('Y-m-d H:i:s',$value['post_date']);
				$value2['title']=$value['gj'];
        ...
        $content = file_get_contents('downloaded.csv');
    ...
    function iframe_f() {
        ...
        if ($mid) {
            ...
            $input = array("text", "radio", "textarea", "select", "opt", "module", "checkbox");
            if ($tmplist) {
                $jslist = $rslist = array();
