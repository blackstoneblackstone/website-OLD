	...
	function baidu_f()
	{
		...
		if($catelist && is_array($catelist) && $cate_count>0)
		{
			foreach($catelist AS $key=>$value)
			{
				...
				$rslist[] = array("title"=>$value["cate_name"],"url"=>$myurl,"datetime"=>$this->system_time,"changefreq"=>"monthly","priority"=>"1");
			}
		}
		$max = 49999 - $cate_count;
		//取得主题列表
		...
		if($sublist && is_array($sublist) && count($sublist)>0)
		{
			foreach($sublist AS $key=>$value)
			{
				...
				$rslist[] = array("title"=>$value["title"],"url"=>$myurl,"datetime"=>$value["post_date"],"changefreq"=>"24","priority"=>"1");
			}
		}
		$xml = '<document xmlns:bbs="http://www.baidu.com/search/bbs_sitemap.xsd">'."\n";
		$xml.= "\t".'<webSite>'.$this->sys_config["siteurl"].'</webSite>'."\n";
		$xml.= "\t".'<webMaster>'.$this->sys_config["smtp_admin"].'</webMaster>'."\n";
		$xml.= "\t".'<updatePeri>24</updatePeri>'."\n";
		foreach($rslist AS $key=>$value)
		{
			$xml.= "\t<item>\n";
