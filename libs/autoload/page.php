...
class page_lib
{
	...
	function page_rewrite($url,$total,$ifarray=false,$ifselect=true)
	{
		...
		if($pageid > 0)
		{
			...
			{
				$array_m++;
				$returnlist[$array_m]["url"] = $pageid > 1 ? $url."-".($pageid-1).".html" : $url.".html";
				$returnlist[$array_m]["name"] = $this->lang["prev"];
				$returnlist[$array_m]["status"] = 0;
			}
		}
		...
		//if($pageid < $total_page)
		{
			$array_m++;
			$t_pageid = ($pageid+1)>1 ? ($pageid+1) : "index";
			$returnlist[$array_m]["url"] = $pageid < $total_page?$url."-".($pageid+1).".html" : $url."-".($pageid).".html";
			$returnlist[$array_m]["name"] = $this->lang["next"];
			$returnlist[$array_m]["status"] = 0;
		}
		$array_m++;
		...
		if($ifarray)
		{
			$returnlist[0]["total"]=$total;//dump($this->psize);
            $returnlist[0]["totalPage"]=$total_page;
            $returnlist[0]["currentPage"]=$pageid;
            $returnlist[0]["inputPage"]="<span> 第 <input type='text' size='2' id='pagenum' /> 页 <input type='button' value='确定' onclick=\"window.location.href='".$url."-'+$('#pagenum').val()+'.html'\" /></span>";
			return $returnlist;
		}
		#[组织样式]
		$msg = "<table class='pagelist' cellpadding='0' cellspacing='1'><tr><td class='n'>".$this->psize."/".$total."</td>";
	...
	function page($url,$total,$ifarray=false,$ifselect=true)
	{
		...
		if($pageid > 0)
		{
			...
			{
				$array_m++;
				$returnlist[$array_m]["url"] =$pageid > 1? $url."pageid=".($pageid-1):$url."pageid=1";
				$returnlist[$array_m]["name"] = $this->lang["prev"];
				$returnlist[$array_m]["status"] = 0;
			}
		}
		...
		{
			$array_m++;
			$returnlist[$array_m]["url"] = $pageid < $total_page?$url."pageid=".($pageid+1):$url."pageid=".$pageid;
			$returnlist[$array_m]["name"] = $this->lang["next"];
			$returnlist[$array_m]["status"] = 0;
		}
		$array_m++;
		//if($pageid != $total_page)
		{
			$returnlist[$array_m]["url"] = $pageid != $total_page?$url."pageid=".$total_page:$url."pageid=".$pageid;
			$returnlist[$array_m]["name"] = $this->lang["last"];
			$returnlist[$array_m]["status"] = 0;
		}
		if($ifarray)
		{
                    ...
                    $returnlist[0]["totalPage"]=$total_page;
                    $returnlist[0]["currentPage"]=$pageid;
                    $returnlist[0]["selectPage"]="<select onchange=\"window.location.href='".$url."pageid='+this.value\">".implode("",$select_option)."</option></select>";
                    $returnlist[0]["inputPage"]="<span> 第 <input type='text' size='2' id='pagenum' /> 页 <input type='button' value='确定' onclick=\"window.location.href='".$url."pageid='+$('#pagenum').val()\" /></span>";
                    //dump($returnlist);
                    return $returnlist;
		}
		#[组织样式]
