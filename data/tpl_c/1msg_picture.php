...
<?php unset($hotpic);?>
<SCRIPT>new dk_slideplayer("#certt",{width:"1000px",height:"129px",fontsize:"12px",time:"5000"});</SCRIPT>
</DIV>
...
<div style="line-height:40px;"><span>您所在的位置：<a href="index.php">首页</a> 
    ...
    <?php $left_catelist = phpok_catelist($cate_rs[parentid] ? $cate_rs[parentid] : $cate_rs[id]);?>
		...
		<ul style=" margin:0 15px;">
			...
			<li style=" text-align:center; border-bottom:1px solid #fff; font-size:13px; font-weight:bold; line-height:30px;"><a href="<?php echo $value[linkurl] ? $value[linkurl] : list_url($value);?>" title="<?php echo $value[cate_name];?>" <?php if($value[id] == $cid){?> style="color:#fff82b;"<?php } ?> class="abc"><?php echo sys_cutstring($value[cate_name]);?></a></li>
			<?php } ?>
		</ul>
		</div>
